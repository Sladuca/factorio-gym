...
class FactorioDevServer:
    def __init__(self, factorio_path="factorio"):
        self.factorio_path = factorio_path
        self.process = None
        self.save_path = Path("saves/test.zip")
        
    def _check_factorio_binary(self):
        """Check if Factorio binary is available"""
        try:
            result = subprocess.run([self.factorio_path, "--version"], 
    ...
    def start_server(self):
        ...
        if not self._check_factorio_binary():
            print(f"Error: Factorio binary '{self.factorio_path}' not found or not working")
...
def main():
    ...
    parser.add_argument("--factorio-path", default=os.environ.get("FACTORIO_PATH", "factorio"),
                       help="Path to Factorio binary")
    parser.add_argument("command", nargs="?", choices=["create", "test"], 
                       help="Command to run")
